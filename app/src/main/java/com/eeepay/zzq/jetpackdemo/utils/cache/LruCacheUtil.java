...
public class LruCacheUtil {
    ...
    //通过 SoftReference 修饰，防止大对象造成OOM
    ...
    private LruCacheUtil() {
        ...
        mMemoryCache = new LruCache<String, SoftReference<Object>>(cacheSize);// 也可以写死 1 * 1024 * 1024
    }
    ...
    public void addObjectToMemoryCache(String key, Object value) {
        SoftReference<Object> objectSoftReference = new SoftReference<>(value);
        mMemoryCache.put(key, objectSoftReference);
    }
    ...
    public Object getObjectFromMemCache(String key) {
        Object value = null;
        SoftReference<Object> reference = mMemoryCache.get(key);
        if (reference != null) {
            value = reference.get();
        }
        return value;
